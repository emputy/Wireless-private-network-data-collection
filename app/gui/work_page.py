...
class WorkPage(QWidget):
    ...
    def _run_analysis_batch(self, s):
        ...
        template = (
            f"你是电力行业无线专网情报分析师。基于下面提供的情报条目（每条含来源/标题/链接），"
            f"产出一份中文情报简报，**严格按下列固定模板输出（Markdown）**。只基于实际提供的情报，不臆造不扩展；"
            f"相关度标【高/中/低】。\n"
            f"# 无线专网情报简报\n"
            f"> 报告周期：{today}　数据范围：N 条强相关情报（450MHz/电力无线专网/国家频谱授用）\n\n"
            f"## 一、核心要点\n- （3~5 条，概括最关键发现）\n\n"
            f"## 二、分阶段洞察\n| 阶段 | 国家/客户 | 关键动态（含日期） | 相关度 |\n|---|---|---|---|\n"
            f"| 阶段1 洞察&意愿 | … | … | 高/中/低 |\n| 阶段2 意愿明确 | … | … | 高/中/低 |\n"
            f"| 阶段3 POC测试 | … | … | 高/中/低 |\n| 阶段4 频谱申请 | … | … | 高/中/低 |\n"
            f"| 阶段5 已获取 | … | … | 高/中/低 |\n| 联盟/友商 | … | … | 高/中/低 |\n\n"
            f"## 三、逐条精要\n| 来源 | 标题/摘要 | 与【电力无线专网/450MHz/频谱授权】关联点 |\n|---|---|---|\n…\n\n"
            f"## 四、趋势研判\n- （2~3 条）\n\n"
            f"## 五、行动建议\n- 面向军团无线Club：…\n- 面向项目工程：…\n\n"
            f"## 六、需人工跟进（可选）\n- …\n\n"
            f"【五阶段客户参考】阶段1：阿曼/巴基斯坦/罗马尼亚/希腊/法国/刚果金/埃塞俄比亚/阿尔及利亚/摩洛哥/喀麦隆/埃及/尼日利亚/智利；"
            f"阶段2：安哥拉/肯尼亚/布基纳法索/马里/科特迪瓦/阿根廷；阶段3：阿联酋/泰国/厄瓜多尔/哥伦比亚；"
            f"阶段4：印尼/斯洛伐克/莫桑比克/几内亚；阶段5：伊拉克/马来西亚/南非。"
        )
        messages = [
